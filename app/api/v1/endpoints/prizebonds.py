...
async def add_prize_bond(request: Request):
    ...
    prize_bond_table.put_item(
        Item={"BondNumber": prize_bond_number, "Username": username}
    )
...
async def delete_prize_bond(request: Request):
    username = get_username(request)
    request_body = await request.json()
    prize_bond_numbers = request_body["bonds"]
    with prize_bond_table.batch_writer() as batch:
        for bond in prize_bond_numbers:
            batch.delete_item(Key={"BondNumber": bond, "Username": username})
